...
import { listBlockComments, createBlockComment, resolveBlockComment, deleteBlockComment } from '@/lib/api';
...
export default function NotesSection({ blockId, onAddNote, refreshTrigger = 0, selectedNoteId, onScrollToEditor, onOpenDetail }: NotesSectionProps) {
        ...
        <div className="flex flex-col h-full bg-background/50">
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-muted-foreground/30 text-center px-6">
                    ...
                    <p className="text-sm font-medium italic mb-1">Sin notas todavía</p>
                    <p className="text-[11px] max-w-[180px]">Usa el botón superior o el menú del editor para crear notas.</p>
                </div>
            ) : (
                <div className="space-y-5 pb-6">
                    {notes.map((note, index) => (
                        <div
                            ...
                            onClick={() => onScrollToEditor?.(note.id)}
                            className={`bg-card border-2 rounded-2xl p-5 transition-all duration-300 relative shadow-sm hover:shadow-xl cursor-pointer group flex flex-col gap-3 ${selectedNoteId === note.id
                                ? 'border-primary ring-4 ring-primary/10 scale-[1.03] z-10'
                                : 'border-border hover:border-primary/40'
                                } ${note.resolved ? 'opacity-50 grayscale' : ''}`}
                        >
                            {/* Note number badge */}
                                ...
                                <div className="flex flex-col gap-1.5 shrink-0 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                    {!note.resolved && (
                                        <>
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); onOpenDetail?.(note, index + 1); }}
