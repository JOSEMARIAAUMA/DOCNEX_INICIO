...
import {
    ...
    Scale, Info, CheckCircle, Trash2, Archive, ExternalLink, Tag
} from 'lucide-react';
...
interface RegulatoryCardProps {
    ...
    onAction?: (action: 'delete' | 'archive' | 'external' | 'edit', res: RegulatoryResource) => void;
    idx?: number;
}
...
export default function RegulatoryCard({ resource, onRead, onAction, idx = 0 }: RegulatoryCardProps) {
    ...
    const handleMenuAction = (action: 'delete' | 'archive' | 'external' | 'edit', e: React.MouseEvent) => {
        e.stopPropagation();
        setIsMenuOpen(false);
    ...
    return (
        ...
        >
            ...
            {/* Veto/Obsolete Badge or Processed % Badge */}
            {isObsolete ? (
            ...
            ) : resource.processed_pct !== undefined && resource.processed_pct > 0 ? (
                <div className="absolute top-4 right-4 z-30 flex items-center gap-1.5 px-2 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full backdrop-blur-sm shadow-sm group-hover:scale-110 group-hover:bg-emerald-500/20 transition-all duration-300">
                    <CheckCircle className="w-2.5 h-2.5 text-emerald-400" />
                    <span className="text-[9px] font-black text-emerald-400 tracking-tight">Procesado {resource.processed_pct}%</span>
                </div>
            ) : null}
                    ...
                    <AnimatePresence>
                        {isMenuOpen && (
                            ...
                            >
                                ...
                                >
                                    <Tag className="w-3.5 h-3.5 text-primary" /> Editar Metadatos
                                </button>
